...
export class ComfyUi {
  ...
  async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
    ...
    const processTextParameter = (value: string): string => {
      return value || '';
    };

    const processNumberParameter = (numberValue: number): number => {
      return numberValue !== undefined ? numberValue : 0;
    };

    const processBooleanParameter = (booleanValue: string | boolean): boolean => {
    ...
    const processImageFromUrl = async (
      ...
      if (!imageUrl) {
        throw new NodeOperationError(this.getNode(), `Node Parameters ${index + 1}: Image URL is required when Image Input Type is set to URL.`);
      }

      if (!validateUrl(imageUrl)) {
        throw new NodeOperationError(this.getNode(), `Node Parameters ${index + 1}: Invalid image URL "${imageUrl}". Must be a valid HTTP/HTTPS URL.`);
      }
      ...
      try {
        ...
        if (!imageResponse || !Buffer.isBuffer(imageResponse)) {
          throw new NodeOperationError(this.getNode(), `Node Parameters ${index + 1}: Failed to download image from URL "${imageUrl}". The server did not return valid image data. Please check the URL and try again.`);
        }

        const imageBuffer = Buffer.from(imageResponse);

        if (imageBuffer.length === 0) {
          throw new NodeOperationError(this.getNode(), `Node Parameters ${index + 1}: Downloaded image from URL "${imageUrl}" is empty. Please check the URL and try again.`);
        }
    ...
    const processImageFromBinary = async (
      paramName: string,
      value: string,
      index: number
    ): Promise<string> => {
      ...
      if (!inputData || !inputData[0] || !inputData[0].binary || !inputData[0].binary[binaryPropertyName]) {
        ...
        throw new NodeOperationError(this.getNode(), `Node Parameters ${index + 1}: Binary property "${binaryPropertyName}" not found in input data.
...
TIP: Check the previous node's "Output Binary Key" parameter. It should match "${binaryPropertyName}".`);
      ...
      if (!binaryData.data || typeof binaryData.data !== 'string') {
        throw new NodeOperationError(this.getNode(), `Node Parameters ${index + 1}: Invalid binary data for property "${binaryPropertyName}". The data field is missing or not a string.`);
      }
      ...
      if (!binaryData.mimeType || typeof binaryData.mimeType !== 'string') {
        throw new NodeOperationError(this.getNode(), `Node Parameters ${index + 1}: Invalid or missing MIME type for binary data.`);
      }
      ...
      if (!Buffer.isBuffer(buffer) || buffer.length === 0) {
        throw new NodeOperationError(this.getNode(), `Node Parameters ${index + 1}: Failed to decode binary data for property "${binaryPropertyName}". The data may be corrupted. Buffer length: ${buffer.length}`);
      }
    ...
    const processImageParameter = async (
      nodeId: string,
      paramName: string,
      imageSource: string,
      imageUrl: string,
      ...
      if (imageSource === 'url') {
        return await processImageFromUrl(paramName, imageUrl, index);
      } else {
        return await processImageFromBinary(paramName, value, index);
      }
    };
      ...
      if (nodeParametersInput && nodeParametersInput.nodeParameter && Array.isArray(nodeParametersInput.nodeParameter)) {
        ...
        for (const [i, nodeParamConfig] of nodeParametersInput.nodeParameter.entries()) {
          ...
          if (parameterMode === 'multiple' && parametersJson) {
            ...
            switch (type) {
              case 'text':
                parsedValue = processTextParameter(value);
                workflow[nodeId].inputs[paramName] = parsedValue;
                break;

              case 'number':
                parsedValue = processNumberParameter(numberValue);
                workflow[nodeId].inputs[paramName] = parsedValue;
                break;

              case 'boolean':
                parsedValue = processBooleanParameter(booleanValue);
                workflow[nodeId].inputs[paramName] = parsedValue;
                break;

              case 'image':
                parsedValue = await processImageParameter(
                  nodeId,
                  paramName,
                  imageSource,
                  imageUrl,
